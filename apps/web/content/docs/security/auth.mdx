...
- The managed **Openship GitHub App private key** lives only on Openship Cloud. A self-hosted operator may
  instead configure their own App key in the API process environment; it never enters the database or a
  dashboard response. Both paths mint short-lived, repository-scoped installation tokens.
